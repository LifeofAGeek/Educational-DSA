...
int f(string R, int n, bool tight){
    ...
    int sum=0;
    loop(dig,0,ub){
        sum+=(dig*cnt(R,n-1,tight&(dig==ub)));
        sum+=f(R,n-1,tight&(dig==ub));
    }
...
void solve(){
    ...
    mem(dp,-1);
    int right=f(r,r.size(),1);
    mem(dp,-1);
    int left=f(l,l.size(),1);
